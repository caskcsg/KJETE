...
def split_and_save(train_docs, dev_docs, data, data_aug, seed, save_dir, nr=0.0):
...
def main(args):
    ...
    data_augumentation = pickle.load(open(args.data_dir + "/dataaugmentation.pickle", "rb"))
    # train_data += data_augumentation
    # train_data = {**train_data, **data_augumentation}
    print("process train ...")
    data = [parallel(v, k, args, tokenizer, bert_model) for k, v in train_data.items()]

    data_aug = [parallel(v, k, args, tokenizer, bert_model) for k, v in data_augumentation.items()]
    ...
    if "all" in args.split:
        ...
        split_and_save(train_docs, dev_docs, data, data_aug, args.seed, args.save_data_dir)
